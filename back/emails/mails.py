...
def encode_mail_params(params_raw):
    return base64.urlsafe_b64encode(zlib.compress(bytes(json.dumps(params_raw), "utf-8"))).decode()


def decode_mail_params(pramas_encoded):
    return json.loads(zlib.decompress(base64.urlsafe_b64decode(pramas_encoded.encode())).decode())
...
def send_email(
    ...
    for to in recivers:
        ...
        try:
            ...
            print(f"User '{to}' doesn't seem to be registered, sending mail anyways")
