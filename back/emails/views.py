...
class ViewEmail(UserPassesTestMixin, View):
    ...
    def test_func(self):
        ...
        if self.request.user.is_staff:
            return True
        if self.request.user.is_authenticated and self.request.user.state.has_extra_user_permission(State.ExtraUserPermissionChoices.EMAIL_TEMPLATES_VIEW):
            return True
        if self.request.user.is_authenticated and self.request.user.state.has_extra_user_permission(State.ExtraUserPermissionChoices.MATCHING_USER):
            # TODO: re-evaluate if matching users should be allowed here?
            return True
        return False

    def get(self, request, **kwargs):
        if "mail_name" not in kwargs:
        ...
        if not parms:
            context_dict = {k: getattr(mail_data.defaults, k) for k in mail_data.defaults.__annotations__}
            ...
            context_dict = {k: getattr(mail_data.texts, k) for k in mail_data.texts.__annotations__}
