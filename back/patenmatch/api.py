...
from patenmatch.models import PatenmatchOrganization, PatenmatchOrganizationUserMatching, PatenmatchUser
...
class PatenmatchUserViewSet(viewsets.ModelViewSet):
        ...
        return Response(
            {
                ...
                "request_specific_organization": PatenmatchOrganizationSerializerCensored(
                    obj.request_specific_organization
                ).data
                if obj.request_specific_organization
                else None,
            }
        )
    ...
    def verify_email(self, request, pk=None):
        ...
        if match is None:
            ...
            return Response({"status": "no-match-found", "match": None}, status=200)

        org = match
        pt_user = user

        PatenmatchOrganizationUserMatching.objects.create(organization=org, user=pt_user)
        ...
        send_email_background.delay(
            "patenmatch-orga-forward-user", user_id=org.id, patenmatch=True, patenmatch_org=True, context=context
        )

        return Response({"status": "ok", "match": PatenmatchOrganizationSerializerCensored(match).data})
