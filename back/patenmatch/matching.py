...
def find_organization_match(user: PatenmatchUser) -> Optional[PatenmatchOrganization]:
    ...
    organizations = PatenmatchOrganization.objects.filter(Q(target_groups__contains=user.support_for)).exclude(
    ...
    for org in organizations:
        distance_score = calculate_distance_score(user.postal_code, org.postal_code, float(org.maximum_distance))
