...
class OutUserWentOnline(MessageBase):
    ...
    def build_redux_action(self):
        return {"action": "updateMatchProfile", "payload": {"partnerId": self.sender_id, "profile": {"isOnline": True}}}
...
class InMatchProposalAdded(MessageBase):
    ...
    def build_redux_action(self):
        return {"action": "addMatch", "payload": {"category": self.category, "match": self.match}}
...
class InBlockIncomingCall(MessageBase):
    ...
    def build_redux_action(self):
        return {"action": "blockIncomingCall", "payload": {"userId": self.sender_id}}
...
class NewActiveCallRoom(MessageBase):
    ...
    def build_redux_action(self):
        return {"action": "addActiveCallRoom", "payload": self.call_room}
...
class PreMatchingAppointmentBooked(MessageBase):
    ...
    def build_redux_action(self):
        return {"action": "preMatchingAppointmentBooked", "payload": self.appointment}
...
class MessagesReadChat(MessageBase):
    ...
    def build_redux_action(self):
        return {"action": "markChatMessagesRead", "payload": {"chatId": self.chat_id, "userId": self.user_id}}
...
def get_all_websocket_callback_messsages(request):
    ...
    for key in annotations:
        list_annotations.append({"type": key, "fields": annotations[key]})
