    ...
    def handle(self, *args, **options):
        ...
        if existing_lobby:
            ...
            if lobby_users_count > 0:
                self.stdout.write(self.style.SUCCESS(f"Deleted {lobby_users_count} lobby user(s)"))

            # Clear all matchings
            matchings_count, _ = RandomCallMatching.objects.filter(lobby=existing_lobby).delete()
            if matchings_count > 0:
                self.stdout.write(self.style.SUCCESS(f"Deleted {matchings_count} matching(s)"))

            # Delete the lobby itself
            existing_lobby.delete()
            self.stdout.write(self.style.SUCCESS("Deleted existing 'default' lobby"))
        else:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Created new 'default' lobby with start_time={lobby.start_time} and end_time={lobby.end_time}"
            )
        )
