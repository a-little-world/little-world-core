...
class Command(BaseCommand):
    def handle(self, **options):
        ...
        non_support_matches = Match.objects.filter(Q(confirmed=True) & Q(active=True) & Q(support_matching=False)).distinct()

        learns_that_had_a_match = User.objects.filter(Q(match_user1__in=non_support_matches) | Q(match_user2__in=non_support_matches)).filter(profile__user_type="learner").distinct().order_by("-date_joined")

        # TODO somethings wrong with this
        learers_that_have_unsubscibed = User.objects.filter(settings__email_settings__unsubscibed_options__contains=["survery_requests"])

        learns_that_have_no_received_prev_suvery_emails = learns_that_had_a_match.filter(~Q(id__in=emails))
        ...
        if user_input == "Y":
            ...
            controller.send_group_mail(users=users, subject="Umfrage zur Verbesserung von Little World", mail_name="general_interview_03_11", mail_params_func=get_params, unsubscribe_group="survery_requests")
        else:
            print("Do nothing...")
