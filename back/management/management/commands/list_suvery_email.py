...
class Command(BaseCommand):
    def handle(self, **options):
        ...
        emails = EmailLog.objects.filter(Q(template__in=prev_suvery_emails) & Q(sucess=True)).values_list(
            "receiver", flat=True
        )
        ...
        learns_that_had_a_match = (
            User.objects.filter(Q(match_user1__in=non_support_matches) | Q(match_user2__in=non_support_matches))
            .filter(profile__user_type="learner")
            .distinct()
        ...
        print(
            "Found",
            len(learns_that_have_no_received_prev_suvery_emails),
            "learners that have not received the prev suvery emails",
        )
