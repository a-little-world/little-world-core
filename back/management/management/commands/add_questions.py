...
class Command(BaseCommand):
    def handle(self, *args, **options):
        if not CardContent.objects.exists():
            with open("/back/management/management/commands/Questions_add.csv", 'r') as csvfile:
                        ...
                        try:
                            ...
                            for key in data["content"].keys():
                                if data["content"][key] != "":
                                    ...
                                    break
                            if data_present:
                                obj = CardContent.objects.create(content=data["content"], category_name = data["name"])
                                print(f"object created  {count}")
                                count += 1
                        except:
        ...
        if CardContent.objects.exists():
            ...
            for user in user_obj:
                ...
                if created:
                    cat_user.categories.set(cat_id)
                    cat_user.save()
                try:
