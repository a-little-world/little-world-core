...
def test_local():
    ...
    for i in range(len(singn_up)):
        for j in range(i+1, len(singn_up)):
            ...
            list1 = set(list(get_user_journey_list(singn_up[i]).queryset(User.objects.all()).values_list('id', flat=True)))
            list2 = set(list(get_user_journey_list(singn_up[j]).queryset(User.objects.all()).values_list('id', flat=True)))
