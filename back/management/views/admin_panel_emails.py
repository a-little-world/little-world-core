...
def render_as_email(request, template_name=None):
    ...
    params_injected_text = inject_template_data(dataclass_as_dict(template.texts), dataclass_as_dict(mail_params))
...
def send_email_rendered(request, template_name=None):
    ...
    for to in receivers:
        ...
        send_email(recivers=[to], subject=subject, mail_data=mail_data, mail_params=params)

        send_emails.append({"to": to, "subject": subject, "template": template_name, "params": mail_params_data.copy()})
    return Response({"status": "ok", "message": "Email sent", "info": send_emails})
