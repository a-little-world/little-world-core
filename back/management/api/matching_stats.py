...
def get_matching_statictic_score_function(request, scoring_function):
    ...
    if scoring_function == ScoringFunctionsEnum.users_waiting_for_match.name:
        requires_matching = needs_matching(qs=User.objects.filter(id__in=request.user.state.managed_users.all(), is_active=True))
        return MatchingStatisticScore(scoring_function=ScoringFunctionsEnum.users_waiting_for_match.value, score_type=ScoreTypesEnum.value.value, data={"value": requires_matching.count()})
    if scoring_function == ScoringFunctionsEnum.learners_waiting_for_match.name:
        ...
        return MatchingStatisticScore(scoring_function=ScoringFunctionsEnum.learners_waiting_for_match.value, score_type=ScoreTypesEnum.value.value, data={"value": learners_needs_matching.count()})

    if scoring_function == ScoringFunctionsEnum.percentage_of_learners_waiting_for_match.name:
        requires_matching = needs_matching(qs=User.objects.filter(id__in=request.user.state.managed_users.all(), is_active=True))
        all_count = requires_matching.count()
        learners_needs_matching = requires_matching.filter(profile__user_type=Profile.TypeChoices.LEARNER)
        return MatchingStatisticScore(scoring_function=ScoringFunctionsEnum.percentage_of_learners_waiting_for_match.value, score_type=ScoreTypesEnum.percentage.value, data={"value": (learners_needs_matching.count() / all_count) * 100})
    if scoring_function == ScoringFunctionsEnum.matchable_scores.name:
        ...
        return MatchingStatisticScore(scoring_function=ScoringFunctionsEnum.matchable_scores.value, score_type=ScoreTypesEnum.value.value, data={"value": count_matchable})
    if scoring_function == ScoringFunctionsEnum.unmatchable_scores.name:
        ...
        return MatchingStatisticScore(scoring_function=ScoringFunctionsEnum.unmatchable_scores.value, score_type=ScoreTypesEnum.value.value, data={"value": count_unmatchable})
    ...
    if scoring_function == ScoringFunctionsEnum.considerable_match_permutations.name:
        ...
        return MatchingStatisticScore(scoring_function=ScoringFunctionsEnum.considerable_match_permutations.value, score_type=ScoreTypesEnum.value.value, data={"value": combinations})

    if scoring_function == ScoringFunctionsEnum.total_matching_score_count.name:
        from management.models.scores import TwoUserMatchingScore

        count = TwoUserMatchingScore.objects.count()
        return MatchingStatisticScore(scoring_function=ScoringFunctionsEnum.total_matching_score_count.value, score_type=ScoreTypesEnum.value.value, data={"value": count})
    else:
