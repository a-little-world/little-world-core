...
def _verify_play_integrity_token(integrity_token: str, request_hash: str) -> bool:
    try:
        _dbg(f"[DEBUG] Starting Play Integrity token verification")
...
def _verify_jwt_token(integrity_token: str, request_hash: str) -> bool:
    try:
        ...
        _dbg(f"[DEBUG] Created PEM key for verification")
        ...
        try:
            ...
            _dbg(f"[DEBUG] JWT verification successful")
...
def _validate_integrity_signals(payload: dict, request_hash: str) -> bool:
    try:
        _dbg(f"[DEBUG] Validating integrity signals")
...
def app_integrity_verify_android(request):
    ...
    """
    _dbg(f"[DEBUG] Android verification request received")
    ...
    _dbg(f"[DEBUG] Starting Android Play Integrity verification")
    ...
    if not _verify_play_integrity_token(integrity_token, request_hash):
        _dbg(f"[ERROR] Android Play Integrity verification failed")
        return Response(
            {"detail": "Android integrity verification failed"}, 
            status=status.HTTP_403_FORBIDDEN
        )
    
    _dbg(f"[SUCCESS] Android Play Integrity verification passed")
    ...
    if not outer_layer_decryption_key:
        _dbg(f"[ERROR] Native app decryption key not configured")
        return Response(
            {"detail": "Native app decryption key not configured"}, 
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
    
    _dbg(f"[SUCCESS] Returning decryption key to client")
    return Response({
...
def app_integrity_verify_ios(request):
    ...
    try:
        attestation_data = json.loads(attestation_object)
    ...
    if getattr(settings, 'PLAY_INTEGRITY_STRICT_MODE', False):
        ...
        return Response({"detail": "iOS attestation strict verification not implemented"}, status=status.HTTP_403_FORBIDDEN)

    cache.delete(cache_key)
    outer_layer_decryption_key = getattr(settings, 'NATIVE_APP_SECRET_DECRYPTION_KEY', None)
    if not outer_layer_decryption_key:
        _dbg("[ERROR] Native app decryption key not configured (iOS)")
        return Response({"detail": "Native app decryption key not configured"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
