...
class TwoUserMatchingScoreFilter(filters.FilterSet):
    ...
    score_gte = filters.NumberFilter(
        ...
        help_text='Filter for scores greater than or equal to'
    )

    score_lte = filters.NumberFilter(
        field_name='score', lookup_expr='lte',
        help_text='Filter for scores less than or equal to'
    )

    matchable = filters.BooleanFilter(
        field_name='matchable',
        help_text='Filter for matchable scores'
    )
    ...
    current_match_suggestion = filters.BooleanFilter(
        method='filter_current_match_suggestion',
        help_text='Filter for current match suggestions'
    )
