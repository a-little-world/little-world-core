...
availabol_model_configs = {
    "mixtral8x7B": {"model": "mistralai/Mixtral-8x7B-Instruct-v0.1", "backend": settings.AI_BASE_URL},
    "gpt-4-turbo": {"model": "gpt-4-1106-preview", "backend": "default"},
    "gpt-4": {"model": "gpt-4", "backend": "default"},
}
...
def request_streamed_api_response(request):
    ...
    if mconf not in availabol_model_configs:
        return Response(
            {"error": f"Invalid model config, possible configs {', '.join(list(availabol_model_configs.keys()))}"},
            status=400,
        )
