...
class MatchFilter(filters.FilterSet):
    ...
    created_between = filters.DateFromToRangeFilter(field_name="created_at", help_text="Range filter for when the match was created, accepts string datetimes")

    updated_between = filters.DateFromToRangeFilter(field_name="updated_at", help_text="Range filter for when the match was last updated, accepts string datetimes")
    ...
    list = filters.ChoiceFilter(field_name="list", choices=[(entry.name, entry.description) for entry in MATCH_JOURNEY_FILTERS], method="filter_list", help_text="Filter for users that are part of a list")
...
class AdvancedMatchViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_staff:
            ...
            return Match.objects.filter(Q(user1__in=user.state.managed_users.all()) | Q(user2__in=user.state.managed_users.all()))
