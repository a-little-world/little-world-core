...
PANEL_V1_FILTER_LISTS = [
    ...
    FilterListEntry("searching", "Users who are searching for a match! Exclude users that have not finished the user form or verified their email!", searching_users),
    FilterListEntry("needs_matching", "All users in 'searching' without any user that has an open proposal!", needs_matching),
    FilterListEntry("in_registration", "Users who have not finished the user form or verified their email!", users_in_registration),
    FilterListEntry("active_within_3weeks", "Users who have been active within the last 3 weeks!", active_within_3weeks),
    FilterListEntry("active_match", "Users who have communicated with their match in the last 4 weeks", get_active_match_query_set),
    FilterListEntry("highquality_matching", "Users who have at least one matching with 20+ Messages", get_quality_match_query_set),
    FilterListEntry("message_reply_required", "Users who have an unread message to the admin user", get_user_with_message_to_admin),
    FilterListEntry("read_message_but_not_replied", "Read messages to the management user that have not been replied to", get_user_with_message_to_admin_that_are_read_but_not_replied),
    FilterListEntry("users_with_open_tasks", "Users who have open tasks", users_with_open_tasks),
    FilterListEntry("users_with_open_proposals", "Users who have open proposals", users_with_open_proposals),
    FilterListEntry("volunteers_booked_onboarding_call_but_never_visited", "Volunteers who have booked an onboarding call but never visited", get_volunteers_booked_onboarding_call_but_never_visited),
    FilterListEntry("users_require_prematching_call_not_booked", "Users that still require a pre-matching call before matching, but haven't booked one yet", users_require_prematching_call_not_booked),
    FilterListEntry("users_with_booked_prematching_call", "Users that have booked a pre-matching call", users_with_booked_prematching_call),
...
USER_JOURNEY_FILTER_LISTS = [
    ...
    FilterListEntry("journey_v2__pre_matching", "(Active-User) User has `Pre-Matching` or `Kickoff-Matching` Match.", pre_matching),
    FilterListEntry("journey_v2__match_takeoff", "(Active-User) User has `Pre-Matching` or `Kickoff-Matching` Match.", match_takeoff),
