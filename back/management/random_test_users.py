...
def random_names(amnt):
    ...
    last_names = ("Johnson", "Aachen", "Smith", "Williams", "Berlin", "Nuggets", "Hendrix")
    names = []
    for i in range(amnt):
...
pics = [f for f in glob.glob("/back/dev_test_data/*") if "management_user" not in f]
...
adv = ("crazily.", "dutifully.", "foolishly.", "merrily.", "occasionally.", "foolishly")
...
def rand_descr(n=1):
    ...
    for i in range(n):
        ...
        t += nouns[num] + " " + verbs[num] + " " + adv[num] + " " + adj[num] + " "
    return t


def random_choice(text_choices):
    return text_choices.values[random.randint(0, len(text_choices.values) - 1)]
...
def create_test_user(i, user_seeds=None, password=None, email=None, pass_if_exists=False, send_verification_mail=False):
    ...
    usr = controller.create_user(**_data, send_verification_mail=send_verification_mail)
    assert usr
    ...
    for choice in user_form_choices:
        setattr(usr.profile, choice, random_choice(user_form_choices[choice]))
    if usr.profile.image_type == Profile.ImageTypeChoice.AVATAR:
        usr.profile.avatar_config = random_avatar()
    else:
        usr.profile.add_profile_picture_from_local_path(pics[random.randint(0, len(pics) - 1)])
